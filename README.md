...
```
WC_STORE_URL=https://wordpress.discoverwhoami.com/
...
## Course Login & Library

The `/course-login` route signs into the LMS REST API and securely stores a token so `/my-courses` can fetch the learner's catalog via `/wp-json/learnpress/v1/courses`.
...
4. After signing in you are redirected to `http://localhost:3000/my-courses`, where the authenticated course list loads automatically. Use “Log out” there to clear the cookie at any time.
